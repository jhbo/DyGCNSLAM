...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, cv::Mat &mask,cv::Mat &imRGBOut,
                                cv::Mat &imDOut, cv::Mat &maskOut)
{
    ...
    if (getenv("USE_ORB") == nullptr)
    {
        ...
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,imMask,_imRGB,mpGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    ...
    mCurrentFrame = Frame(mImGray,imDepth,timestamp,imMask,imRGBOut,mpGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
                                 ...
                                 cv::Mat &mask)
{
    ...
    mCurrentFrame = Frame(mImGray,imDepth,timestamp,imMask,mpGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    //cout<< "mImRGB" << this->mImRGB << "mImDepth" << this->mImDepth <<endl;
